...
const NavigationItem = ({ item, isActive, onClick, isCollapsed }) => {
  ...
  return (
    ...
    >
      <Icon
        className={`size-5 shrink-0 ${isActive ? "text-blue-600" : "text-gray-500"}`}
      />
...
const DashboardLayout = ({ activeMenu, children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50  transition-transform duration-300 transform ${isMobile ? (sidebarOpen ? "translate-x-0" : "-translate-x-full") : "translate-x-0"} ${sidebarCollapsed ? "w-16" : "w-64"} bg-white border-r border-gray-200`}
      >
        {/* company logo */}
        ...
        <div className="flex items-center h-16 border-b border-gray-200 pl-6">
          {!sidebarCollapsed ? (
            <Link to="/" className="flex items-center space-x-3">
              <div className="size-8 bg-linear-to-br from-blue-600 to-blue-700 rounded-lg flex justify-center items-center ">
                <Briefcase className="" />{" "}
              </div>{" "}
            ...
            <div className="size-8 bg-linear-to-r from-blue-600 to-purple-600 rounded-xl flex items-center justify-center ">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <button
            onClick={logout}
            className="w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-xl text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-300"
          >
